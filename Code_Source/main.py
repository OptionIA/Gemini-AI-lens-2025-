...
TOOLS = [
    {
        "google_search": {}  # Google search tool configuration
    },
    {
...
# Configuracion para la generacion de contenido usando diccionarios planos
CONFIG = {
    "generation_config": {
        "temperature": 0.7,
        "top_p": 0.9,
            ...
            async with (
                ...
                for task_name, coro in tasks:
                    if task_name == "send_text":
                        send_text_task = tg.create_task(task_wrapper(coro, task_name))
                    else:
                        tg.create_task(task_wrapper(coro, task_name))
