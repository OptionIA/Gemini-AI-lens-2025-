...
def get_language():
    """
    Obtiene el idioma configurado desde config.json
    """
    try:
        with open("data/config.json", "r", encoding="utf-8") as f:
            config = json.load(f)
            return config.get("language", "es-US")  # default to es-US if not set
    except Exception:
        return "es-US"  # return default if any error occurs
